...
const (
	...
	// KGroupKeyLogWorker is group key for log-worker block in defaults.yaml. This is the parent key. All the nested
	...
	KGroupKeyLogWorker = "logsubscriber"

	// KLogsDirectory is a nested key under the group key KGroupKeyLogWorker to obtain the logs_directory.
	KLogsDirectory = KGroupKeyLogWorker + ".logs_directory"

	// KSanitizedLogsDirectory is a nested key under the group KGroupKeyLogWorker to obtain the directory in which the
	// sanitized files are written.
